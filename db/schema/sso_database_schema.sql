...
-- 包含: 基础表结构 + 安全增强 + 第三方登录 + MD5+盐值密码支持
-- ========================================
...
CREATE TABLE sys_user (
                          ...
                          salt VARCHAR(32) NOT NULL COMMENT '密码盐值，32位随机字符串，用于MD5加密',
...
CREATE TABLE sys_menu (
    ...
    status CHAR(1) DEFAULT '1' COMMENT '菜单状态：0-禁用，1-正常',
...
CREATE TABLE user_password_history (
                                       id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT 'ID',
                                       user_id BIGINT NOT NULL COMMENT '用户ID',
                                       password_hash VARCHAR(32) NOT NULL COMMENT '密码哈希值（MD5+盐值）',
...
CREATE TABLE sso_client (
    ...
    allowed_origins TEXT COMMENT '允许的来源域名，多个用逗号分隔',
    access_token_validity INT DEFAULT 7200 COMMENT 'Access Token有效期（秒）',
    refresh_token_validity INT DEFAULT 604800 COMMENT 'Refresh Token有效期（秒）',
...
-- 19. SSO授权码表
CREATE TABLE sso_auth_code (
    id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '授权码ID',
    code VARCHAR(100) NOT NULL UNIQUE COMMENT '授权码',
    user_id BIGINT NOT NULL COMMENT '用户ID',
    client_id VARCHAR(100) NOT NULL COMMENT '客户端标识',
    redirect_uri VARCHAR(500) COMMENT '重定向URI',
    scope VARCHAR(200) COMMENT '授权范围',
    state VARCHAR(200) COMMENT '状态参数',
...
SELECT 'SSO数据库结构创建完成！' as message,
       'MD5+盐值密码方案已启用' as password_scheme,
       '包含完整的17个核心表+4个SSO表+视图' as table_info,
       '请执行初始化数据脚本' as next_step;
